<template>
  <div id="app">
    <section>
      <h1 class="subtitle" v-if="events == null">
        Sorry, you must complete a workout first!
      </h1>
    ...
    <ul v-if="selectedDateEvents != null">
      <li v-for="workout in selectedDateEvents" :key="workout.workoutRef.id">
        <SingleWorkout
...
export default {
  ...
  async mounted() {
    ...
    this.$store.commit(
      ...
      dbHistory.data().completedWorkouts,
      {
        root: true,
      }
    );
  ...
  computed: {
    events() {
      if (this.completedWorkouts == null) return null;
      ...
      for (let workout of this.completedWorkouts) {
        ...
        if (
          !Object.keys(names).includes(workoutName) &&
          ...
          // no colors available and workout doesnt already have a color. just assign one
          finalColor = "is-info";
        } else if (Object.keys(names).includes(workoutName)) {
    ...
    selectedDateEvents() {
      if (this.completedWorkouts == null) return null;
      return this.events.filter((elt) =>
        datesAreOnSameDay(elt.date, this.selectedDate)
      );
    },
  },
};
